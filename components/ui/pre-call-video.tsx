...
export function PreCallVideo() {
  ...
  useEffect(() => {
    ...
    const startParam =
      PRE_CALL_VIDEO_START_SECONDS > 0
        ? `&start=${PRE_CALL_VIDEO_START_SECONDS}`
        : '';
    setEmbedSrc(
      `https://www.youtube-nocookie.com/embed/${PRE_CALL_VIDEO_ID}?enablejsapi=1&controls=0&disablekb=1&fs=0&rel=0&modestbranding=1&iv_load_policy=3&playsinline=1&autoplay=0${startParam}&origin=${origin}`
    );
  }, []);
  ...
  const togglePlayback = () => {
    ...
    if (!hasStartedRef.current && PRE_CALL_VIDEO_START_SECONDS > 0) {
