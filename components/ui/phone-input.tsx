...
export function PhoneInput({
  ...
  React.useEffect(() => {
    if (value && value !== phoneNumber) {
      // Only update if the value prop has a country code
      const countryMatch = countries.find((c) => value.startsWith(c.phoneCode));
      if (countryMatch && value.length > countryMatch.phoneCode.length) {
        setSelectedCountry(countryMatch);
        setPhoneNumber(value.slice(countryMatch.phoneCode.length).trim());
      } else if (!value.startsWith('+')) {
        // If no country code, just set the number without changing country
  ...
  return (
    ...
    >
      ...
      <Input
        ...
        disabled={disabled}
        className={cn(
          "border-none bg-transparent p-0 shadow-none focus-visible:ring-transparent focus-visible:border-transparent focus-visible:outline-transparent font-sans",
          showValidation &&
            phoneNumber.length > 0 &&
